...
// ── GOVERNMENT FEES ──────────────────────────────────────────────────────
// No live-app precedent for this section — govt_fees is a rate/config table
// with no management UI in the original app at all (see KNOWN_GAPS.md).
// Kept deliberately simple: name + amount, same pattern as a custom charge.

export async function saveGovtFee(
  id: string | null,
  feeName: string,
  amount: number,
) {
  const user = await requireOwner();
  if (!feeName.trim()) return fail("Fee name is required.");
  const supabase = await createClient();
  const payload = {
    dive_center_id: user.diveCenterId,
  ...
  const { error } = id
    ? await supabase.from("govt_fees").update(payload).eq("id", id)
    : await supabase.from("govt_fees").insert(payload);
  if (error) return fail(error.message);
  return ok();
}

export async function deleteGovtFee(id: string) {
  const user = await requireOwner();
  const supabase = await createClient();
  const { error } = await supabase
    .from("govt_fees")
